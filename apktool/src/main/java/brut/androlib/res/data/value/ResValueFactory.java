...
import android.util.TypedValues;
...
public class ResValueFactory {
    ...
    public ResScalarValue factory(int type, int value, String rawValue) throws AndrolibException {
        switch (type) {
            case TypedValues.TYPE_NULL:
                if (value == TypedValues.DATA_NULL_UNDEFINED) { // Special case $empty as explicitly defined empty value
                    return new ResStringValue(null, value);
                } else if (value == TypedValues.DATA_NULL_EMPTY) {
                    return new ResEmptyValue(value, rawValue, type);
                }
                return new ResReferenceValue(mPackage, 0, null);
            case TypedValues.TYPE_REFERENCE:
                return newReference(value, rawValue);
            case TypedValues.TYPE_ATTRIBUTE:
                return newReference(value, rawValue, true);
            case TypedValues.TYPE_STRING:
                return new ResStringValue(rawValue, value);
            case TypedValues.TYPE_FLOAT:
                return new ResFloatValue(Float.intBitsToFloat(value), value, rawValue);
            case TypedValues.TYPE_DIMENSION:
                return new ResDimenValue(value, rawValue);
            case TypedValues.TYPE_FRACTION:
                return new ResFractionValue(value, rawValue);
            case TypedValues.TYPE_INT_BOOLEAN:
                return new ResBoolValue(value != 0, value, rawValue);
            case TypedValues.TYPE_DYNAMIC_REFERENCE:
                return newReference(value, rawValue);
            case TypedValues.TYPE_DYNAMIC_ATTRIBUTE:
                return newReference(value, rawValue, true);
        }

        if (type >= TypedValues.TYPE_FIRST_COLOR_INT && type <= TypedValues.TYPE_LAST_COLOR_INT) {
            return new ResColorValue(value, rawValue);
        }
        if (type >= TypedValues.TYPE_FIRST_INT && type <= TypedValues.TYPE_LAST_INT) {
